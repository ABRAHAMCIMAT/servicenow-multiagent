...
2. En Jan, añade un **proveedor personalizado** (Settings → Advanced → OpenAI-compatible API):
   ...
   - **API Key:** `jan` (cualquier valor) si el backend corre sin `API_KEY` configurada (modo demo). Si configuraste `API_KEY` en el backend, usa ese mismo valor aquí — Jan lo envía como `Authorization: Bearer <valor>`, que el backend valida.
