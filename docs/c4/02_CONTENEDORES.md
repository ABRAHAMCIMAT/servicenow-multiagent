...
C4Container
    ...
    System_Boundary(sistema, "ServiceNow Multi-Agent System") {
        Container(api, "API Server", "Python 3.12 / FastAPI", "REST API, endpoint OpenAI-compatible, orquestación de los 8 agentes, auth, rate limiting")
        ...
        ContainerDb(telemetria, "Telemetría", "Archivo JSONL", "Eventos estandarizados: trace_start/end, agent_span, llm_call, conversation, rag_hit, approval, escalation")
    }
...
| **Almacenamiento de conversaciones** | JSON en disco (`backend/data/conversations.json`) | Historial de conversaciones, detrás de `ConversationStorage` (Protocol, ver Nivel 4) | — |
| **Telemetría** | JSONL en disco (`backend/data/telemetry.jsonl`) | Log de eventos append-only que alimenta `MetricsEngine` | — |
...
- **Seguridad por configuración, no por diseño de contenedores**: no hay un
  API Gateway/proxy separado — `API_KEY`, rate limiting y CORS se aplican
  dentro del propio API Server (`llmops/security.py`), suficiente a esta
  escala; ver [07_GUARDRAILS.md](../llmops/07_GUARDRAILS.md).
