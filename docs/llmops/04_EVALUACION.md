...
- Catálogo de objetivos vía `patterns.Registry`.
- Sin fallback determinista ante error del LLM: la generación creativa de
  casos no tiene un sustituto heurístico razonable; los errores del
  proveedor se propagan como `ProviderError`/`RetryableError`.
- Disponible como CLI (`scripts/generate_test_matrix.py`) y como API
  (`GET /api/test-matrix/targets`, `POST /api/test-matrix`, rol `agent`).
...
- La suite de `pytest` (`tests/unit/`, `tests/integration/`, `tests/e2e/`)
  cubre el resto de agentes y el flujo completo; corre en CI en cada push/PR.
