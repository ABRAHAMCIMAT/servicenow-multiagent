...
def test_returns_openai_shape(app, user_key):
    r = _client(app, user_key).post("/v1/chat/completions",
                                    json={"messages": [{"role": "user",
                                                        "content": "no puedo entrar al CRM"}]})
...
def test_includes_id_model_and_usage(app, user_key):
    body = _client(app, user_key).post(
        "/v1/chat/completions",
...
def test_uses_last_user_message(app, user_key):
    body = _client(app, user_key).post(
        "/v1/chat/completions",
        json={"messages": [{"role": "user", "content": "mensaje viejo"},
...
def test_streaming_chunks_are_valid_json(app, user_key):
    ...
    payloads = [l[6:] for l in r.text.splitlines() if l.startswith("data: ") and l != "data: [DONE]"]
    assert payloads
    for p in payloads:
...
def test_user_role_is_enough(app, user_key):
    ...
    assert _client(app, user_key).post(
        "/v1/chat/completions",
        json={"messages": [{"role": "user", "content": "hola"}]}).status_code == 200


def test_conversation_is_persisted(app, user_key):
    _client(app, user_key).post("/v1/chat/completions",
                                json={"messages": [{"role": "user", "content": "hola"}]})
