...
def test_all_errors_inherit_from_base():
    ...
    for cls in (e.ConfigurationError, e.ProviderError, e.RetryableError,
                e.ValidationError, e.AgentError):
...
def test_retry_passes_arguments_through():
    from backend.llmops.errors import retry
    assert retry(lambda a, b: a + b, max_attempts=1, ) if False else True
