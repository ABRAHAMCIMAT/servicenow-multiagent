...
class TracedCoordinator:
    ...
    def handle(self, user_message: str, caller: str = "Usuario") -> Conversation:
        ...
        # Escalation event — summary_sent refleja si el agente de escalación
        # realmente construyó y adjuntó el resumen ejecutivo (mensaje
        # "escalacion" en la traza), no una suposición a partir del status.
        if escalated:
            summary_sent = any(m.agent == "escalacion" for m in conv.messages)
            self._telemetry.escalation(trace_id, "L2", ticket_number, summary_sent=summary_sent)
