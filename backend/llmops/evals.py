...
def check_llm_judge(llm: Any, criteria: str, field_name: str = "answer") -> Callable[[dict], EvalResult]:
    """Evaluación LLM-as-judge (extensible, ver docs/llmops/04_EVALUACION.md).
    ...
    def _check(output: dict) -> EvalResult:
        ...
        if not text:
            return EvalResult(
                name="llm_judge", passed=False, score=0.0, details=f"campo '{field_name}' vacío"
            )
        prompt = (
        ...
        try:
            ...
            log.warning("llm_judge no disponible; se omite sin bloquear el flujo", extra={"error": str(e)})
            return EvalResult(name="llm_judge", passed=True, score=0.5, details=f"evaluación omitida: {e}")
