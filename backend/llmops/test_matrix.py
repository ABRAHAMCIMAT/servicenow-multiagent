...
def get_target(key: str) -> TestTarget:
    try:
        return cast(TestTarget, _targets.get(key))
    except KeyError:
        raise KeyError(f"Objetivo de prueba no registrado: '{key}'. Disponibles: {list_targets()}") from None
...
register_target(
    TestTarget(
        key="clasificador",
        description=(
            "Agente Clasificador (backend/agents/classifier.py): recibe un mensaje "
...
register_target(
    TestTarget(
        ...
        description=(
            "Validación de entrada del sistema (llmops/guardrails.py): "
            "check_max_length (2000 caracteres) y check_prompt_injection."
        ),
        input_description=(
            "Un mensaje de texto que pondría a prueba los límites de longitud "
...
register_target(
    TestTarget(
        key="conocimiento",
        description=(
            "Agente de Conocimiento / RAG (backend/agents/knowledge.py): busca en la "
...
register_target(
    TestTarget(
        key="api_chat",
        description=(
            "Endpoint POST /api/chat (backend/server.py): recibe {message, caller} y "
...
register_target(
    TestTarget(
        ...
        description=(
            "Endpoint POST /v1/chat/completions (backend/server.py), compatible con "
...
def generate_batch(llm: Any, target_key: str, count: int = MAX_BATCH_SIZE) -> dict:
    ...
    if len(raw_cases) > MAX_BATCH_SIZE:
        log.warning(
            "el LLM devolvió más casos que el límite; se truncó el lote",
    ...
    log.info("matriz de pruebas generada", extra={"agent": target_key, "status": "ok", "tokens": len(cases)})
