...
Determinista por diseño (no invoca al LLM): el diagnóstico alimenta
directamente la matriz de políticas de aprobación (agents/policy.py), y en
un sistema de ITSM real se prefiere que esa cadena sea auditable y
reproducible en vez de generada por un modelo. `check_json_schema` se
...
class DiagnosticAgent:
    def __init__(self, snow):
        ...
        self.evaluator.register("json_schema", lambda o: check_json_schema(
            o, ["root_cause", "account_state", "recommended_action"]))
