...
from ..llmops.errors import safe_call, ValidationError
...
class ClassifierAgent:
    ...
    def classify(self, user_message: str) -> Classification:
        ...
        # Guardrail de salida — valida intent/priority antes de continuar;
        # si el LLM devolvió valores fuera de esquema, degrada a la
        # heurística determinista en vez de propagar datos inválidos.
        try:
            data = default_guardrails.validate_output(data)
        except ValidationError as e:
            log.warning("salida del LLM rechazada por guardrails; degradando a heurística",
                        extra={"error": str(e)})
            data = self._heuristic(user_message)
        ...
        if not all(r.passed for r in eval_results):
            ...
            log.warning("evaluación de salida falló; degradando a heurística",
    ...
    def _heuristic(self, msg: str) -> dict:
        ...
        # El check de "estado" va antes que el genérico de "cómo": un mensaje
        # como "¿cómo va mi ticket?" contiene "cómo", así que si el check de
        # conocimiento fuera primero, el caso de estado del test set
        # (04_EVALUACION.md) nunca se alcanzaría. Mismo orden que en
        # MockLLM._mock_reply (core/llm.py) por la misma razón.
