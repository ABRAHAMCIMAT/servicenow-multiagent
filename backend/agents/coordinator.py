...
class CoordinatorAgent:
    def __init__(self, llm: LLM, snow: ServiceNowAdapter, notifier: NotificationAdapter):
        ...
        self.diagnostic = DiagnosticAgent(snow)
        self.policy = PolicyAgent()
        self.execution = ExecutionAgent(snow)
    ...
    def _run(self, agent_name: str, fn, *args, **kwargs):
        ...
        try:
            ...
            log.error(f"fallo inesperado en agente '{agent_name}'",
    ...
    def _handle_resolution(self, conv: Conversation) -> Conversation:
        ...
        if not result.get("success", True):
            conv.add("ejecucion", f"⚠️ {result.get('message', 'No se pudo completar la acción automáticamente.')}",
                     data=result)
            return self._escalate(conv)
