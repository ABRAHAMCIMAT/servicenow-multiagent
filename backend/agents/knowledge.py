...
# (nunca debería ocurrir; ver llmops/prompts.py).
_FALLBACK_PROMPT = (
...
class KnowledgeAgent:
    def __init__(self, llm: LLM, snow):
        ...
        # LLM-as-judge (Fase 4): opcional, cuesta una llamada LLM extra por
        # consulta — se activa explícitamente con ENABLE_LLM_JUDGE=true.
        if os.getenv("ENABLE_LLM_JUDGE", "").lower() in ("1", "true", "yes"):
            self.evaluator.register("llm_judge", check_llm_judge(
                llm,
                criteria=("la respuesta explica la solución paso a paso, en español, "
    ...
    def answer(self, conv: Conversation) -> dict:
        ...
        # (llmops/prompts.py) en vez de un duplicado hardcodeado aquí.
        try:
