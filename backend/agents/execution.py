...
Determinista por diseño (no invoca al LLM): ejecuta acciones reales sobre
cuentas y licencias, así que el enrutamiento acción→operación debe ser
predecible y auditable, no generado por un modelo. `check_json_schema` se
ejecuta igual sobre cada resultado como red de seguridad de regresión.
"""
...
log = get_logger("agente.ejecucion")
...
class ExecutionAgent:
    def __init__(self, snow):
        ...
        self.evaluator.register("json_schema", lambda o: check_json_schema(
            o, ["success", "action", "message"]))

    def execute(self, conv: Conversation, action: str, target: str = "") -> dict:
        result = self._execute(conv, action, target)
        for r in self.evaluator.run(result):
            if not r.passed:
                log.warning("evaluación de salida falló en ejecución", extra={"error": r.details})
        return result

    def _execute(self, conv: Conversation, action: str, target: str = "") -> dict:
