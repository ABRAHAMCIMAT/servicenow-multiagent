...
from ..llmops.errors import ConfigurationError, ProviderError, RetryableError, retry
    ...
    def from_env(cls) -> "LLMConfig":
        ...
        if provider == "openai":
            ...
            if not os.getenv("LLM_API_KEY"):
                # Fail-fast: Jan ignora la API key, pero OpenAI real la exige.
                # Sin esto, el error solo aparecería como un 401 confuso en
                # la primera llamada.
                raise ConfigurationError(
                    "LLM_PROVIDER=openai requiere la variable de entorno "
                    "LLM_API_KEY con una clave válida."
                )
...
class MockLLM:
    ...
    def _mock_reply(self, user: str, system: str = "") -> str:
        ...
        if "clasificador" in s or "clasificar" in s or "intencion" in s:
            ...
            # Nota: el check de "estado" va antes que el genérico de "cómo"
            # porque "¿cómo va mi ticket?" contiene "cómo" — sin este orden,
            # el patrón genérico de conocimiento lo capturaba primero y el
            # caso de estado del test set (04_EVALUACION.md) nunca se
            # alcanzaba (bug detectado por scripts/run_evals.py).
        ...
        if "evaluador de calidad" in s:
            return json.dumps({"passed": True, "score": 0.85,
                                "reason": "Respuesta clara y basada en el artículo (mock)."})
        ...
        if "generador de matrices de pruebas" in s:
            ...
            for i in range(count):
                ...
                if t == "positive":
                    ...
                    inp = "x" * 2000
                    behavior = "Debe manejarse en el límite sin romper el flujo (truncar, rechazar o procesar)."
                cases.append({"type": t, "input": inp, "expected_behavior": behavior,
...
class OpenAICompatLLM:
    """Cliente HTTP hacia un servidor OpenAI-compatible (Jan, OpenAI, etc.).

    Los errores transitorios (timeout, fallo de red, 429, 5xx) se reintentan
    con backoff exponencial + jitter (`retry()`, llmops/errors.py). Los
    demás errores del proveedor se traducen a `ProviderError` para que el
    resto del sistema pueda distinguirlos de un bug propio del agente.
    """
    ...
    def complete(self, messages, **kw) -> str:
        ...
        def _call() -> str:
            try:
                ...
                # timeout, DNS, conexión rechazada, etc. — transitorio
                raise RetryableError(f"error de red hacia el proveedor LLM: {e}") from e

            if resp.status_code == 429 or resp.status_code >= 500:
                raise RetryableError(f"el proveedor LLM devolvió {resp.status_code}")
            if resp.status_code >= 400:
                raise ProviderError(
                    f"el proveedor LLM devolvió {resp.status_code}: {resp.text[:300]}"
                )
            try:
                data = resp.json()
...
class LLM:
    def __init__(self, config: Optional[LLMConfig] = None):
        ...
        self._impl = _provider_strategy.execute(key, self.config)
